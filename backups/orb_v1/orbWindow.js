...
function registerPipelineHandler() {
  ...
  ipcMain.on("orb:move", (_e, { x, y }) => {
    ...
    const targetDisplay = cachedDisplays.find(d =>
        x >= d.workArea.x && x <= d.workArea.x + d.workArea.width &&
        y >= d.workArea.y && y <= d.workArea.y + d.workArea.height
    ) || screen.getPrimaryDisplay();
    const { workArea } = targetDisplay;
    const { width, height } = orbWindow.getBounds();
    const newX = Math.round(Math.max(workArea.x, Math.min(x, workArea.x + workArea.width - width)));
    const newY = Math.round(Math.max(workArea.y, Math.min(y, workArea.y + workArea.height - height)));
...
export function showOrb(x, y) {
  if (!orbWindow) {
    ...
    // Additional IPC handler to toggle mouse ignoring based on renderer hover state
